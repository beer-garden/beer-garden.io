== Example 5: User Forms and Advance Features
:includedir: _includes

We have now covered the basic of how to create Beer Garden Plugins with a variety of Input Types and Output Types.
...
image::{includedir}/complex-beer-statistics-test.png[Complex Beer Statistics Results]
...
class ComplexBeerStatisticsClient:
    ...
    @parameter(
        ...
        choices={"type": "command", "value": "valid_year_range"},  # (2)
    )
...
image::{includedir}/complex-beer-statistics-years.png[Complex Beer Statistics Years]

We can then go to the `Requests` page and see that the `Request` was submitted to invoke that command
to populate the form.

image::{includedir}/complex-beer-statistics-years-request.png[Complex Beer Statistics Years Request]
...
class ComplexBeerStatisticsClient:
    ...
    @parameter(
        ...
        type="Integer",
        description="Enter the year to pull Beer Production Statistics",
        choices={"type": "command", "value": "valid_date_ranges"},
    )
    def test_beer_statistics_client_with_year(self, year):
        results = self.beer_statistics_client.download_year_statistics(year=year)
        return results.output
    ...
    @parameter(
        ...
        choices={"type": "command", "value": "valid_summary_fields"}, # (2)
    )
...
image::{includedir}/complex-beer-statistics-fields.png[Complex Beer Statistics Fields]
...
class ComplexBeerStatisticsClient:
    ...
    @parameter(
        ...
        type="String",
        description="Enter the field to pull Beer Production Statistics on",
        choices={"type": "command", "value": "valid_summary_fields"},
    )
    def test_beer_statistics_client_fields(self, field):
        return field
...
image::{includedir}/complex-beer-statistics-final-request.png[Complex Beer Statistics Fields]
...
image::{includedir}/complex-beer-statistics-final-response.png[Complex Beer Statistics Fields]
