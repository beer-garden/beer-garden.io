...
CAUTION: We are working on getting a yum repository hosted through https://packagecloud.io/[Package cloud, title=Package cloud], so hopefully we can point you at a yum repo sometime soon! For now you'll have to download an RPM from our GitHub releases section.
...
All Beer Garden configuration files are located in `/opt/beer-garden/conf`. Feel free to adjust the configuration as necessary. For more information on configuration see link:../../configuration/[Configuring Beer Garden, title=Configuring Beer Garden].
