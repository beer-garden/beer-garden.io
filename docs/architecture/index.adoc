...
=== Frontend
The Beer Garden frontend is an AngularJS application. It's built with ``yarn`` and ``webpack`` and served by any hosting platform. We have examples on hosting the Frontend through NGINX and NPM.

=== Backend
The Beer Garden backend. It's responsible for several things:
...
Beer Garden receives instructions through it's Entry Points (REST). It will always be between UI and Mongo/RabbitMQ interactions.
...
Beer Garden then  *validates* the request (either locally or on child Beer Garden). This validation is significatly more involved than the preliminary validation done by Brew View. Here are some of the validation steps:
...
When Beer Garden receives an initializion request it:
