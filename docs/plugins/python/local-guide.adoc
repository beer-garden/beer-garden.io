...
The goal of this section is to help you write a plugin from scratch. In order to write plugins there are a few things that you need.
...
Once you are done configuring everything, all you need to do is tar the entire folder.
[source]
----
tar -czvf my_plugin.tar.gz my_plugin
----
...
[source]
----
cp my_plugin.tar.gz /opt/beer-garden/plugins/
cd /opt/beer-garden/plugins/
tar -zxvf my_plugin.tar.gz
----
...
Everything in the `my_plugin/` directory is specific module level code you would like to include in your distribution.

==== \\__main__.py

This is what starts up the plugin and keeps it running.

==== \\__init__.py

This is a standard Python module file.  It may be empty, but must exist.
...
Otherwise, if you are using print, the `Plugin` will flush once your method has been completed.
...
Now, you need to include this in your source distribution by modifying or creating your `MANIFEST.in` in your top-level directory and add the following line:
